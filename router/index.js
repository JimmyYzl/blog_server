...
router.post('/article_blog', (req, res) => {
	let body = req.body;
	if (body.title && body.description && body.isshow != undefined && body.article && body.date) {
		if (body.id) {
	...
	if(req.query.id) {
		articleModel.findById(req.query.id, (err, result) => {
			if(err) {
				...
				if(result) {
					articleModel.findByIdAndUpdate(req.query.id,{$set: {numofviews: result.numofviews + 1 || 1}}, (err, r) => {
						console.log(r);
					})
					res.json(result);
				} else {
